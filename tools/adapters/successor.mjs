...
export const read = {
  ...
  quotedPair: (page) => page.evaluate((root) => { const q = document.querySelectorAll(root + " blockquote"); if (q.length < 2) return { quotes: q.length }; const textLeft = (el) => { const t = document.createTreeWalker(el, NodeFilter.SHOW_TEXT).nextNode(); const r = document.createRange(); r.selectNodeContents(t); return r.getBoundingClientRect().left; }; const p = { left: textLeft(q[0]) }, cells = [...q[1].querySelectorAll(".vrow > *")]; const c0 = cells[0].getBoundingClientRect(); const line = parseFloat(getComputedStyle(cells[0]).lineHeight); return { quotes: q.length, indent: Math.round(c0.left - p.left), cols: getComputedStyle(cells[0].parentElement).gridTemplateColumns.split(" ").map((w) => Math.round(parseFloat(w))).join(" "), wrapped: cells.some((c) => c.getBoundingClientRect().height > line * 1.5), sameBox: Math.round(q[1].getBoundingClientRect().left - q[0].getBoundingClientRect().left) === 0 && Math.round(q[1].getBoundingClientRect().width - q[0].getBoundingClientRect().width) === 0 }; }, "#editor"),
