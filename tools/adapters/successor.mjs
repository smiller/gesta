...
export const read = {
  ...
  /* the grid's readings (2026-09-22): the layout, the band above its corner, a selection across its cards, the corner's text */
  gridLayout: (page) => page.evaluate(() => { const g = document.querySelector("#editor div.grid"); if (!g) return { grid: false, columns: 0, cards: document.querySelectorAll("#editor [class^='card-']").length, wider: false, type: null }; const cs = getComputedStyle(g); return { grid: true, columns: cs.gridTemplateColumns.split(" ").length, cards: g.children.length, wider: g.getBoundingClientRect().width > document.querySelector("#editor .ProseMirror").getBoundingClientRect().width, type: cs.fontSize }; }),
  gridHover: async (page) => { const r = await page.evaluate(() => { const g = document.querySelector("#editor div.grid"); return g ? g.getBoundingClientRect().toJSON() : null; }); if (!r) return { show: false, label: null, title: null }; await page.mouse.move(r.right - 30, r.top - 12); await page.waitForTimeout(150); return page.evaluate(() => { const b = document.querySelector(".copybtn"); return { show: b.classList.contains("show"), label: b.textContent, title: b.title }; }); },
  clipboardText: (page) => page.evaluate(async () => { try { return (await navigator.clipboard.readText()).slice(0, 40); } catch (e) { return String(e); } }),
  selectCards: (page) => page.evaluate(() => { const cards = document.querySelectorAll("#editor [class^='card-']"); const r = document.createRange(); r.setStart(cards[0], 0); r.setEnd(cards[cards.length - 1], cards[cards.length - 1].childNodes.length); const s = document.getSelection(); s.removeAllRanges(); s.addRange(r); }),
  ...
  waitPairGrew: (page, prev, ms, phrase) => page.waitForFunction(([root, prev, phrase]) => { const rows = [...document.querySelectorAll(root + " blockquote:has(.vrow) .vrow")]; const row = rows.find((r) => r.textContent.includes(phrase)); if (!row) return false; const cells = [...row.children], line = parseFloat(getComputedStyle(cells[0]).lineHeight); const wrapped = cells.some((x) => x.getBoundingClientRect().height > line * 1.5); return !wrapped && getComputedStyle(row).gridTemplateColumns.split(" ").map((w) => Math.round(parseFloat(w))).join(" ") !== prev; }, ["#editor", prev, phrase], { timeout: ms, polling: "raf" }).then(() => true, () => false),
  quotedPair: (page) => page.evaluate((root) => { const quotes = document.querySelectorAll(root + " blockquote").length; const plain = document.querySelector(root + " blockquote:not(:has(.vrow))"), paired = document.querySelector(root + " blockquote:has(.vrow)"); if (!plain || !paired) return { quotes, plain: !!plain, paired: !!paired }; const textLeft = (el) => { const t = document.createTreeWalker(el, NodeFilter.SHOW_TEXT).nextNode(); if (!t) return null; const r = document.createRange(); r.selectNodeContents(t); return r.getBoundingClientRect().left; }; const left = textLeft(plain), cells = [...paired.querySelectorAll(".vrow > *")], c0 = cells[0]?.getBoundingClientRect(); if (left == null || !c0) return { quotes, text: left != null, cells: cells.length }; const line = parseFloat(getComputedStyle(cells[0]).lineHeight), a = plain.getBoundingClientRect(), b = paired.getBoundingClientRect(); return { quotes, indent: Math.round(c0.left - left), cols: getComputedStyle(cells[0].parentElement).gridTemplateColumns.split(" ").map((w) => Math.round(parseFloat(w))).join(" "), wrapped: cells.some((c) => c.getBoundingClientRect().height > line * 1.5), sameBox: Math.round(b.left - a.left) === 0 && Math.round(b.width - a.width) === 0 }; }, "#editor"),
