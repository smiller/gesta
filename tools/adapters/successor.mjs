...
export const read = {
  ...
  waitPairGrew: (page, prev, ms) => page.waitForFunction(([root, prev]) => { const c = document.querySelector(root + " blockquote:has(.vrow) .vrow > *"); return !!c && getComputedStyle(c.parentElement).gridTemplateColumns.split(" ").map((w) => Math.round(parseFloat(w))).join(" ") !== prev; }, ["#editor", prev], { timeout: ms, polling: "raf" }).then(() => true, () => false),
