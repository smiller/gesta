...
export const read = {
  ...
  bookmarks: (page) => page.evaluate(() => { const c = document.getElementById("bookmarkspanel"); return c && !c.hidden ? { rows: [...c.querySelectorAll("li")].map((li) => li.textContent.replace(/\s+/g, "").slice(0, 40)), foot: c.querySelector(".bookmarks-add")?.textContent.replace(/\s+/g, " ").trim(), focused: document.activeElement?.className.split(" ")[0] } : null; }),
  ...
  highlightFollowed: (page) => page.evaluate(() => ({ selected: document.getSelection().toString(), bar: document.querySelector(".fmt").classList.contains("show") })),
  ...
  search: (page) => page.evaluate(() => ({ open: document.getElementById("pagesearch")?.open, focused: document.activeElement?.className, scope: document.getElementById("pagesearchscope")?.selectedOptions[0]?.textContent, options: [...document.querySelectorAll("#pagesearchscope option")].map((o) => o.textContent), rows: [...document.querySelectorAll("#pagesearchresults li")].map((li) => li.className + ": " + li.textContent.replace(/\s+/g, "").trim()) })),
