...
answer = "Ideas";
await page.click(".toolbtn[title='Create a tagged sub-entry for this day']");
await page.waitForFunction(() => document.documentElement.dataset.entry === "2026-09-06/Ideas", null, { timeout: 5000 });
console.log("+ tagged entry Ideas:", JSON.stringify({ entry: await page.evaluate(() => document.documentElement.dataset.entry), crumb: await page.evaluate(() => document.querySelector(".datelabel").textContent.replace(/\s+/g, " ").trim()), tools: await tools() }));
await page.goto(PAGE + "#2026-09-06");
