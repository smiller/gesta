...
- 2026-09-22, THE TEST GATES. FOUND, asked "what ensures the tests run
  ...
  parse — SETTLED the same day, asked: left as it is; the input is the
  harness's own JSON, generated by code with its own tests. Under `npm run verify`, so the pre-commit runs it.
